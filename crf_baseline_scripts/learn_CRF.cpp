...
int main(int argc, char * argv[]){
  ...
  int where_w1 = 0;
  while(where_w1 < argc && strcmp(argv[where_w1],"-W")!=0 )
    where_w1++;
  if(where_w1 == argc)
    throw MyException("Error! no weight flag found!");
  int where_w1_end = where_w1+1;
  while(where_w1_end < argc && argv[where_w1_end][0] != '-')
    where_w1_end++;
  ...
  cout << "reading models and creating message structures... " << endl << endl; 
  cout.flush();
  ...
  #pragma omp parallel for
    for(int n=0; n<nmodels; n++){
      ...
      //cout << "Input file: " << n << ", Number of model nodes: " << nnodes << " Number of data nodes: " << to_string(y[n].size()) << endl;
	...
	write_params(argv[where_w1+1],W);
